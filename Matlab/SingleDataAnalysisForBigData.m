function [ data, Vmean, Verror, Cv, Cverror ] = SingleDataAnalysisForBigData( outputfile, rows, column, temperature , Np)
...
%   [data gr Vmean Verror Cv Cverror Pmean Perror] = SingleDataAnalysis('outfile', 'g(r)_outfile', 2.0, 100);
    ...
    % Import the data
    data = ImportTooBigData(outputfile, rows, column);
    
    % Save the workspace, because it could crash later
    save(strcat('workspace',num2str(column)))
